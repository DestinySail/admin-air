...
- `apps/web`：Vue 3 + TypeScript + Vite 的管理端前端
- `apps/mock-server`：本地开发时使用的 Hono + TypeScript mock 后端
...
- `apps/web/src/router/index.ts`：hash 路由、鉴权守卫、按路由懒加载语言
- `apps/web/src/lang/index.ts`：i18n 初始化与按需语言加载
- `apps/web/src/stores`：Pinia store 与持久化插件用法
...
- `apps/mock-server/src/index.ts`：内存型 mock API、筛选、排序、分页、树结构辅助、统一响应结构
...
- `pnpm build`：构建前端，并执行 mock server 的 TypeScript 检查
...
- `eslint.config.js`：TypeScript + Vue + Prettier 的 flat config，Vue / TS 规则相对宽松，但 `@typescript-eslint/no-unused-vars` 仍然会报警告
...
- i18n 相关修改应继续放在 `apps/web/src/lang` 下的按需加载结构中
...
- 先阅读根目录 `package.json`、目标应用的 `package.json` 以及相关配置文件
- 判断改动属于 `apps/web`、`apps/mock-server`，还是两端联动
- 查看已有协作文档，例如 `CLAUDE.md`，但如果它与源码冲突，应以源码和配置为准
- 在改变模式、命名或架构前，先阅读周边实现

修改前端时：

- 从最接近需求的 route、view、store 或共享 utility 入手
- 保持现有鉴权流程不变：需要登录的路由在没有 token 时跳转到 `adminLogin`
- 保持 `apps/web/src/router/index.ts` 与 `apps/web/src/lang/index.ts` 中的路由守卫和按需加载语言逻辑
- 保持 `apps/web/src/main.ts` 中的全局样式入口和 Element Plus 集成方式一致
- 调整别名、代理或构建行为前，先检查 `apps/web/vite.config.ts`

修改 API 行为或数据契约时：

- 必要时同步更新 `apps/mock-server/src/index.ts` 与 `apps/mock-server/src/mock-data.ts`
...
当前仓库根目录没有专门配置完整自动化测试套件，因此 lint、类型检查和构建验证是主要质量关卡。汇报结果时，要明确写出实际执行过的命令，以及哪些检查被跳过或无法执行。

## 代理行为

- 优先做精确、最小化的改动，避免大范围重写
- 除非任务明确要求，否则不要主动重构架构、命名或目录结构
- 如果只改一端但可能影响前后端契约，要主动确认另一端是否也需要调整
- 当文档与源码冲突时，以源码为准，并在安全且相关时同步更新文档
- 如果用户再次要求初始化项目说明，先重新扫描仓库，再以增量方式更新本文件，而不是替换成通用模板
