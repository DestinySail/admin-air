...
对于用户可见的前端改动、路由或鉴权变更，以及前后端联动改动，在任务算作完成前必须执行基于浏览器的 MCP E2E 验证。

- 优先使用当前可用的浏览器类 MCP 工具，例如 Chrome DevTools MCP。
- 要验证受影响的真实用户流程，而不只是页面能否打开。
- 如果因为环境或依赖阻塞无法完成 MCP E2E，必须明确说明阻塞原因，不能把任务描述为“已完整验证”。

当前仓库没有自带完整的自动化 E2E 测试套件，因此 lint、类型检查、构建验证，以及必做的浏览器类 MCP E2E 检查，是主要质量关卡。汇报结果时，要明确写出实际执行过的命令、完成了哪些 MCP E2E 场景验证，以及哪些检查被跳过或无法执行。
