...
const state: {
    ...
    charts: [],
    remark: '开源等于互助，开源需要大家一起支持，支持的方式有很多种，比如使用、推荐、写教程、保护生态、贡献代码、回答问题、分享经验、打赏赞助等。',
    workingTimeFormat: '',
    pauseWork: false,
})
...
const startWork = () => {
    ...
    const startPauseTime = workingTime.startPauseTime <= 0 ? 0 : time - workingTime.startPauseTime
...
const formatSeconds = (seconds: number) => {
    ...
    if (dayTime > 0) {
        return dayTime + t('dashboard.day') + result
    }
    return result
}
